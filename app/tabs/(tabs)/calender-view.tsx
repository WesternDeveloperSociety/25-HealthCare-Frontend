...
export default function CalendarView() {
  ...
  const getAppointmentsForDate = (date: string) => {
    const appointments = appointmentsByDate[date as keyof typeof appointmentsByDate] || [];
                  ...
                  <HStack className="items-center space-x-4">
                    ...
                    <Box 
                      className={`px-2 py-1 rounded-full ${
                        appointment.type === 'appointment' ? 'bg-blue-100' :
                        appointment.type === 'document' ? 'bg-green-100' : 'bg-yellow-100'
                      }`}
                    >
                      <Text 
                        className={`text-xs font-medium ${
                          appointment.type === 'appointment' ? 'text-blue-700' :
