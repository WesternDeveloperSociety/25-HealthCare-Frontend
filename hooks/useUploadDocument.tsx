...
export function useUploadDocument() {
  ...
  return useMutation({
    mutationFn: async (file: { uri: string; name: string; type: string; [key: string]: any }) => {
      ...
      if (file.appointmentID) formData.append('appointmentID', file.appointmentID);
      ...
      toast.show({
        render: () => (
          <Toast action="error" variant="solid">
            ...
            <ToastDescription>{error.message || 'Upload failed'}</ToastDescription>
          </Toast>
        ),
      });
    },
